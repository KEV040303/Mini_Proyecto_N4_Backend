  ...
  try {
    ...
    if (!contraseñaValida)
      return res
        ...
        .json({
          message:
            "La contraseña debe tener mayuscula, minusculas y 8 caracteres como mínimo.",
        });
    ...
    return res
      .status(201)
      .json({ message: "Usuario registrado con éxito", token, data: newUser });
  } catch (error) {
    if (error?.errno === 1062)
      return res.status(400).json({ message: "Credenciales duplicadas" });
...
export const login = async (req, res) => {
  try {
    ...
    if (!email || !password)
      return res
        .status(400)
        .json({ message: "Faltan datos para el inciar sesión" });
  ...
  try {
    ...
    if (result) {
      if (prevUserData.profile_picture !== "defaultProfilePicture.jpg") {
        await fs.unlink(
          path.normalize(`uploads/${prevUserData.profile_picture}`)
        );
      }
      return res
        .status(200)
        .json({ message: "Usuario actualizado exitosamente", data: result });
    }
